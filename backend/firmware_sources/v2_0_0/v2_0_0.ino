...
const char* current_version = "2.0.0"; // Version 2.0.0
...
void setup() {
    ...
    Serial.println("\n[BOOT] ========================================");
    Serial.println("[BOOT] ESP32 Production OTA Client: Version 2.0.0");
    ...
    Serial.println("[BOOT] Behaviour: Fast Blinking LED (200ms) & Mock Telemetry");
    Serial.println("[BOOT] ========================================");
...
void loop() {
    ...
    // Version 2.0.0 behavior: Fast blink (every 200 ms)
    ...
    // Telemetry reporting simulation: every 3 seconds
    if (millis() - lastSensorTime >= 3000) {
        lastSensorTime = millis();
        float t = 24.0 + random(-10, 10) / 10.0;
        float h = 60.0 + random(-30, 30) / 10.0;
        Serial.printf("[v2.0.0] Telemetry: Temp=%.1f C, Humidity=%.1f %%\n", t, h);
    }
